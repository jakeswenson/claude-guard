...
- `sexp.rs`, `syntax.rs`, `cond.rs`, `load.rs`: the rule language. Reader with positions; forms to a typed table (`rule`, rows, patterns, `command` and `fact` declarations, facts read before rules); conditions with three-valued evaluation and reasons; which files load and how declarations merge.
- `facts.rs` and `facts/*.rs`: every fact a condition can name, one type per fact behind the `Fact` trait, in a `Facts` registry the conditions consult by name, memoized per fact, arguments, and cwd. Built-ins are `ancestor_has.rs` and `under.rs`; `exec.rs` is a `(fact ...)` declaration's program over stdio with a timeout; `stubs.rs` stands in by name for the spec and the tests, so nothing there touches the disk.
