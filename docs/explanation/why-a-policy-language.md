...
Strip Prolog of compound terms, unbounded recursion, and cut, and what remains is Datalog: facts, predicates, rules, all decidable. That is the shape here. `(ancestor-has? ".jj")` is a fact about the world; `(under? ?p "/tmp")` is a predicate over a value the pattern bound; a rule row is a Horn clause whose head is a decision. The vocabulary is borrowed on purpose, so the design does not have to invent names for things that have them.
...
A condition is true, false, or unknown. Nothing produces unknown today, but the seam is there because the next predicates will: one that asks an external program can time out, and one that depends on the working directory can find it ambiguous after `cd $DIR`.
...
`not` swaps true and false and leaves unknown alone. The two rows that matter for a guard: `false and unknown` is false, because one failed condition settles an `and` whatever the other says, and `true or unknown` is true for the same reason. Everything else that touches an unknown stays unknown, so a timed-out predicate inside an `and` never turns into a silent pass. The design's next step turns "unknown on a matched pattern" into an ask that tells the model what was unknown, so it can remove the ambiguity itself.
...
Square brackets mark a term pattern, so `[git -... stash ...]` reads as the shell line it matches and looks different from a predicate. Binders take a leading `?`, the Datalog convention; predicates take a trailing `?`, the Scheme convention. The two never collide.
